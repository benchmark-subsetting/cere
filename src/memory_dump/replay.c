...
void load(char* loop_name, int invocation, int count, void* addresses[count]) {
    ...
    /* Load adresses */
    snprintf(path, sizeof(path), "cere_dumps/%s/%d/core.map", loop_name, invocation);
    ...
    if (!loaded) {
        ...
        snprintf(path, sizeof(path), "cere_dumps/%s/%d/hotpages.map", loop_name, invocation);
    ...
    while ((ent = readdir (dir)) != NULL) {
        ...
        snprintf(filename, sizeof(filename), "cere_dumps/%s/%d/%s", loop_name, invocation, ent->d_name);
