...
def memdumps_to_objects(dir):
    ...
    # Get the list of all relevant .memdump files
    # (we will only want to use files generated by compress just before)

    # First, get all dump files ...
    dumps = []
    for f in os.listdir("."):
        if f.endswith(".memdump"):
            ...
            + " {dump}.memdump {dump}.o"
            + " --rename-section .data=.{dump},CONTENTS,ALLOC,LOAD,READONLY,DATA").format(
            objcopy=OBJCOPY, dump=f.split(".")[0]))
