...
def run(args):
    ...
    if not run_shell_command("{0} CERE_MODE=original -B".format(cere_configure.cere_config["build_cmd"])):
        return False
    ...
    try:
        logger.debug(subprocess.check_output("{0} CERE_MODE=\"replay --region={1}\" -B".format(cere_configure.cere_config["build_cmd"], args.region), stderr=subprocess.STDOUT, shell=True))
    except subprocess.CalledProcessError as err:
        logger.error(str(err))
        logger.error(err.output)
