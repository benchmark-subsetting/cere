...
def run(args):
  ...
  try:
    logger.debug(subprocess.check_output("{0} MODE=\"original --hybrid --regions-file={1}\" -B".format(cere_configure.cere_config["build_cmd"], args.regions_file), stderr=subprocess.STDOUT, shell=True))
